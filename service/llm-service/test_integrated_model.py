...
def generate_response_fast(prompt, tokenizer, model, max_length=512):
    ...
    with torch.no_grad():
        outputs = model.generate(
            **inputs,
            max_length=max_length,      # 빠른 생성
            temperature=0.3,            # 빠른 결정
            top_p=0.7,                  # 적당한 다양성
            do_sample=False,            # 탐욕적 생성, 빠름
...
def test_integrated_model():
    """통합 모델 테스트 (기존 GRI + 한온시스템 + HL만도)"""
    print("🚀 통합 모델 테스트 시작!")
    print("(기존 GRI + 한온시스템 + HL만도 데이터 모두 학습된 모델)")
    ...
    try:
        ...
        print(f"\n{'='*80}")
        print("🎉 모든 통합 모델 테스트 완료!")
        print("기존 GRI + 한온시스템 + HL만도 데이터를 모두 학습한 통합 모델이 준비되었습니다!")
        print(f"{'='*80}")
