            ...
            if extra_instructions:
                ...
                try:
                    ...
                    if meta_start != -1 and meta_end != -1:
                        ...
                        if meta.get("company_context") == "true" and meta.get("corporation_name"):
                            company_context = (
                                f"중요: 이 응답은 {meta['corporation_name']} 기업의 ESG 보고서를 위한 것입니다. "
                                f"모든 'ABC', '회사', '조직' 등의 표현을 '{meta['corporation_name']}'으로 대체하고, "
                                f"기업 특성에 맞게 응답을 조정하세요."
                            )
                            logger.info(f"✅ 회사이름 적용됨: {meta['corporation_name']}")
                            logger.info(f"Created company context: {company_context}")
                        else:
                            logger.warning(f"⚠️ 회사 컨텍스트 누락: company_context={meta.get('company_context')}, corporation_name={meta.get('corporation_name')}")
                except Exception as e:
